...
- [ ] [QA-03] Create `src/auth/tokens.ts` with `generateAccessToken()` and `generateRefreshToken()` ← current
...
> QA-01 and QA-02 completed. Auth middleware in `src/middleware/auth.ts`
> and User model in `src/models/user.ts` with Prisma schema are done.
> Next: QA-03 — token generation utilities (`generateAccessToken()` and
> `generateRefreshToken()`) in `src/auth/tokens.ts`.
