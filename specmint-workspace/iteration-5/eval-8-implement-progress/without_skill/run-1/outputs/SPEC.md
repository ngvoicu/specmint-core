...
> Completed QA-01 (`src/middleware/auth.ts` -- JWT verification middleware)
> and QA-02 (`src/models/user.ts` -- User interface + Prisma schema).
> Next: QA-03 -- create `src/auth/tokens.ts` with `generateAccessToken()`
> and `generateRefreshToken()` helper functions.
